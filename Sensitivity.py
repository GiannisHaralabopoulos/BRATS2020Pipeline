...
DEFAULT_CFG = {
    ...
    # HybridAttUNet pipeline-sensitivity study
    # The reference network uses a 128^3 internal grid. Its deepest attention
    # branch requires at least 128 voxels per axis if the architecture itself is
    # to remain unchanged. Therefore the low-resolution condition reduces the
    # information to 96^3 and then resamples it back to the unchanged 128^3
    # network grid before the first learned layer.
    "hybridatt_reference_internal_size": 128,
    "hybridatt_low_effective_size": 96,
    "hybridatt_high_internal_size": 160,
...
def move_image_to_device(image: torch.Tensor, device: torch.device, cfg: dict) -> torch.Tensor:
    ...
    if (
        ...
        and image.dim() == 5
        and cfg.get("model", "").lower() in {"unet3d", "hybridattunet", "hybridatt_lowres",
            "hybridatt_highres", "hybridatt_ref_noaug", "deepensemble"}
    ):
...
# ── HybridAttUNet pipeline sensitivity ──────────────────────────────────────

class HybridAttUnetLowResolution3D(HybridAttUnet3D):
    """
    Low-information-resolution sensitivity condition.

    The original HybridAttUNet architecture is left unchanged at its 128^3
    working grid. The incoming full-volume tensor is first resampled to 96^3,
    removing spatial information, and then resampled to 128^3 before entering
    the unchanged network.

    A direct 96^3 execution is intentionally not used: the deepest residual
    attention branch contains four successive 2x pooling operations and would
    require changing its topology/pooling behaviour at 96^3. This wrapper tests
    pipeline sensitivity to lower spatial information without confounding the
    result with an architectural modification.
    """
    def __init__(
        self,
        in_channels: int = 4,
        ...
        self.effective_input_size = int(effective_input_size)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        original_shape = tuple(x.shape[2:])
        low_shape = (self.effective_input_size,) * 3
        work_shape = (self.internal_size,) * 3

        if tuple(x.shape[2:]) != low_shape:
            x = F.interpolate(
                x, size=low_shape, mode="trilinear", align_corners=False
            )

        # Return to the original architecture's valid 128^3 working grid. This
        # restores tensor dimensions, not information lost at the 96^3 step.
        if tuple(x.shape[2:]) != work_shape:
            x = F.interpolate(
                x, size=work_shape, mode="trilinear", align_corners=False
            )

        # Because x is already 128^3, the parent model performs no additional
        # input resizing and therefore executes the exact reference network.
...
HYBRIDATT_SENSITIVITY_MODELS = (
    "hybridatt_lowres",
    "hybridatt_highres",
    "hybridatt_ref_noaug",
)

HYBRIDATT_SENSITIVITY_SPECS = {
    "hybridatt_lowres": {
        "label": "Low Resolution",
        "internal_size": 128,
        "effective_input_size": 96,
        "augment": True,
        "description": (
            "96^3 effective spatial information resampled to the unchanged "
            "128^3 HybridAttUNet working grid"
        ),
    },
    "hybridatt_highres": {
        "label": "High Resolution",
        "internal_size": 160,
        "effective_input_size": 160,
        "augment": True,
        "description": "160^3 HybridAttUNet internal working grid",
    },
    "hybridatt_ref_noaug": {
        "label": "Reference No Augmentation",
        "internal_size": 128,
        "effective_input_size": 128,
        "augment": False,
        "description": (
            "Reference 128^3 HybridAttUNet with the training augmentation "
            "pipeline disabled"
        ),
    },
}
...
def apply_hybridatt_sensitivity_config(cfg: dict) -> dict:
    """Apply the immutable definition of one HybridAttUNet sensitivity arm."""
    name = str(cfg.get("model", "")).lower()
    if name not in HYBRIDATT_SENSITIVITY_SPECS:
        return cfg

    spec = HYBRIDATT_SENSITIVITY_SPECS[name]
    cfg["hybridatt_internal_size"] = int(spec["internal_size"])
    cfg["hybridatt_effective_input_size"] = int(spec["effective_input_size"])
    cfg["augment"] = bool(spec["augment"])
    cfg["sensitivity_condition"] = str(spec["label"])
    cfg["sensitivity_description"] = str(spec["description"])
    return cfg
...
MODEL_REGISTRY = {
    ...
    "hybridatt_lowres":     HybridAttUnetLowResolution3D,
    "hybridatt_highres":    HybridAttUnet3D,
    "hybridatt_ref_noaug":  HybridAttUnet3D,
    "unet3d":          UNet3D,
...
def build_model(cfg: dict) -> nn.Module:
    ...
    if name in HYBRIDATT_SENSITIVITY_MODELS:
        kwargs["internal_size"] = int(
            cfg.get(
                "hybridatt_internal_size",
                cfg.get("hybridatt_reference_internal_size", 128),
            )
        )
        if name == "hybridatt_lowres":
            kwargs["effective_input_size"] = int(
                cfg.get(
                    "hybridatt_effective_input_size",
                    cfg.get("hybridatt_low_effective_size", 96),
                )
            )
...
class Trainer:
    def __init__(
        ...
        if (
            ...
            and cfg["model"].lower() in {"unet3d", "hybridattunet", "hybridatt_lowres",
            "hybridatt_highres", "hybridatt_ref_noaug", "deepensemble"}
        ):
    ...
    def _autotune_batch_size(self, requested_batch: int) -> int:
        ...
        if cfg.get("model") in HYBRIDATT_SENSITIVITY_MODELS:
            diff_revision += (
                f"|hybridsens_v1"
                f"|internal{int(cfg.get('hybridatt_internal_size', 128))}"
                f"|effective{int(cfg.get('hybridatt_effective_input_size', 128))}"
...
class FixedSplitRunner:
    ...
    def run(self):
        ...
        if str(cfg.get("model", "")).lower() in HYBRIDATT_SENSITIVITY_MODELS:
            sensitivity_row = dict(combined_row)
            sensitivity_row["Model"] = HYBRIDATT_SENSITIVITY_SPECS[
                str(cfg["model"]).lower()
            ]["label"]
            sensitivity_path = (
                self.save_dir / "hybridattunet_sensitivity_metrics.csv"
            )
            self._upsert_model_row(sensitivity_path, sensitivity_row)
            logger.info(
                f"HybridAttUNet sensitivity metrics -> {sensitivity_path}"
            )
        ...
        run_summary = {
            ...
            "sensitivity_condition": (
                cfg.get("sensitivity_condition")
                if str(cfg.get("model", "")).lower()
                in HYBRIDATT_SENSITIVITY_MODELS
                else None
            ),
            "hybridatt_internal_size": (
                int(cfg.get("hybridatt_internal_size", 128))
                if str(cfg.get("model", "")).lower()
                in HYBRIDATT_SENSITIVITY_MODELS
                else None
            ),
            "hybridatt_effective_input_size": (
                int(cfg.get("hybridatt_effective_input_size", 128))
                if str(cfg.get("model", "")).lower()
                in HYBRIDATT_SENSITIVITY_MODELS
                else None
            ),
            "training_augmentation_enabled": bool(cfg.get("augment", True)),
...
class CrossValidationRunner:
    ...
    def _eval_checkpoint_on_test(
        ...
        if (
            ...
            and self.cfg["model"].lower() in {"unet3d", "hybridattunet", "hybridatt_lowres",
            "hybridatt_highres", "hybridatt_ref_noaug", "deepensemble"}
        ):
...
MODEL_MENU = [
    (
        "hybridatt_lowres",
        "Low Resolution - 96^3 effective information, unchanged 128^3 network grid",
    ),
    (
        "hybridatt_highres",
        "High Resolution - 160^3 internal working grid",
    ),
    (
        "hybridatt_ref_noaug",
        "Reference No Augmentation - 128^3 internal grid, augmentation disabled",
    ),
]
...
def parse_args():
    p = argparse.ArgumentParser(
        description="HybridAttUNet BraTS2020 Pipeline Sensitivity Study",
    ...
    p.add_argument(
        "--model",
        default=None,
        choices=list(HYBRIDATT_SENSITIVITY_MODELS),
        help=(
            "HybridAttUNet sensitivity condition. Interactive training prompts "
            "by default; use --no_model_prompt to bypass it."
        ),
    )
    p.add_argument(
        "--run_all_sensitivity",
        action="store_true",
        help=(
            "Run Low Resolution, High Resolution and Reference No Augmentation "
            "sequentially on the same fixed patient split."
        ),
    )
...
def main():
    ...
    if args.list_models:
        ...
        descriptions = {
            key: (
                ...
                f"(augmentation={'on' if spec['augment'] else 'off'})"
            )
            for key, spec in HYBRIDATT_SENSITIVITY_SPECS.items()
        }
    ...
    if args.mode == "train" and args.run_all_sensitivity:
        # Use the first arm while common data/cache setup is performed. Each
        # actual training run receives its own immutable arm configuration.
        args.model = HYBRIDATT_SENSITIVITY_MODELS[0]
    ...
    # Sensitivity-arm definitions take precedence over generic augmentation
    # flags. Low/high resolution always use the reference augmentation policy;
    # the no-augmentation reference always disables it.
    cfg = apply_hybridatt_sensitivity_config(cfg)
    ...
    if bool(getattr(args, "run_all_sensitivity", False)):
        if cfg.get("cv"):
            raise RuntimeError(
                "--run_all_sensitivity uses the fixed 80/10/10 experiment, not CV."
            )
        if cfg["mode"] != "train":
            raise RuntimeError(
                "--run_all_sensitivity is available only in training mode."
            )
        ...
        manifest["common_controls"] = {
            "fixed_split_file": str(cfg.get("fixed_split_file")),
            "seed": int(cfg.get("seed", 123)),
            "loss": "0.5 foreground Dice + 0.5 categorical cross-entropy",
            "optimizer": "AdamW",
        ...
        manifest_path = (
            Path(cfg["save_dir"]) / "hybridattunet_sensitivity_manifest.json"
        )
        with open(manifest_path, "w", encoding="utf-8") as f:
            json.dump(manifest, f, indent=2, default=str)
        logger.info(f"Sensitivity manifest -> {manifest_path}")

        for index, model_key in enumerate(
            HYBRIDATT_SENSITIVITY_MODELS, start=1
        ):
            variant_cfg = dict(cfg)
            variant_cfg["model"] = model_key
            variant_cfg["checkpoint"] = None
            variant_cfg = apply_hybridatt_sensitivity_config(variant_cfg)

            spec = HYBRIDATT_SENSITIVITY_SPECS[model_key]
            logger.info("")
            logger.info("#" * 78)
            logger.info(
                f"HYBRIDATTUNET SENSITIVITY {index}/3: {spec['label']}"
            )
            logger.info(
                f"{spec['description']} | augmentation="
                f"{'ON' if spec['augment'] else 'OFF'}"
            )
            logger.info("#" * 78)
        ...
        logger.info(
            "HybridAttUNet sensitivity study complete. Combined results -> "
            f"{Path(cfg['save_dir']) / 'hybridattunet_sensitivity_metrics.csv'}"
        )
    elif cfg.get("cv"):
