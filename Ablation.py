...
def move_image_to_device(image: torch.Tensor, device: torch.device, cfg: dict) -> torch.Tensor:
    ...
    if (
        ...
        and image.dim() == 5
        and cfg.get("model", "").lower() in {"unet3d", "hybridattunet", "hybridattunet_no_attention",
            "hybridattunet_no_residual",
            "hybridattunet_no_attention_no_residual", "deepensemble"}
    ):
...
# ── HybridAttUnet ablation variants ──────────────────────────────────────────

class HARPlainBlock3D(nn.Module):
    """
    Non-residual counterpart of HARResidualBlock3D.

    It preserves the residual block's three-stage pre-activation bottleneck
    main path (1x1x1, 3x3x3, 1x1x1) but removes the identity/projection shortcut
    and the final shortcut addition.  This isolates the contribution of
    residual learning without widening the main convolutional pathway.
    """
    def __init__(self, in_channels: int, out_channels: int):
        super().__init__()
        mid_channels = max(out_channels // 4, 8)
...
class HARSoftMaskBranchAblation3D(nn.Module):
    """
    Soft-mask branch with a selectable residual or non-residual convolutional
    block.  Spatial depth and mask-head construction are identical to the full
    HybridAttUnet attention branch.
    """
    def __init__(self, channels: int, depth: int, block_cls, r: int = 1):
        super().__init__()
        self.depth = int(depth)
        self.down_blocks = nn.ModuleList([
            nn.Sequential(*[
                block_cls(channels, channels) for _ in range(r)
            ])
            for _ in range(self.depth)
        ])
        self.bottom = nn.Sequential(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for block, skip in zip(self.up_blocks, reversed(skips)):
            y = F.interpolate(
                y, size=skip.shape[2:], mode="trilinear",
                align_corners=False,
            )
            y = y + skip
            y = block(y)
        ...
        if y.shape[2:] != x.shape[2:]:
            y = F.interpolate(
                y, size=x.shape[2:], mode="trilinear",
                align_corners=False,
            )
        return self.mask_head(y)


class HARAttentionAblation3D(nn.Module):
    """
    Full residual-attention mechanism with a selectable convolutional block.

    When block_cls is HARPlainBlock3D, attention is retained while residual
    shortcuts inside the convolutional blocks are removed.
    """
    def __init__(
        self,
        channels: int,
        ...
        super().__init__()
        self.pre = nn.Sequential(*[
            block_cls(channels, channels) for _ in range(p)
        ])
        self.trunk = nn.Sequential(*[
            block_cls(channels, channels) for _ in range(t)
        ])
        self.soft_mask = HARSoftMaskBranchAblation3D(
            channels, depth=depth, block_cls=block_cls, r=r
        )
        self.post = block_cls(channels, channels)
...
class HARNoAttentionSkip3D(nn.Module):
    """
    Attention-removed skip processor.

    The non-mask pre/trunk/post pathway is retained so the ablation does not
    collapse each skip connection to a raw identity operation.  The soft-mask
    branch and multiplicative attention gate are removed completely.
    """
    def __init__(
        self,
        channels: int,
...
class HybridAttUnetAblation3D(nn.Module):
    """
    Controlled ablation implementation derived from the exact HybridAttUnet3D
    used in the primary benchmark.

    The stem, channel widths, encoder/decoder hierarchy, 128^3 internal working
    volume, interpolation, bottleneck layout and final segmentation head are
    held fixed.

    use_attention=False removes:
      * the encoder-decoder soft-mask attention branches and multiplicative
        residual-attention gates
      * all decoder squeeze-excitation channel-attention modules

    use_residual=False replaces every HARResidualBlock3D used by the
    encoder, bottleneck, decoder and skip-processing pathway with
    HARPlainBlock3D, which keeps the same three-convolution main path but
    removes residual shortcut addition.
    """
    def __init__(
        self,
        in_channels: int = 4,
        ...
        block_cls = HARResidualBlock3D if self.use_residual else HARPlainBlock3D

        self.stem = nn.Conv3d(
            in_channels, 32, kernel_size=3, padding=1, bias=True
        )

        self.enc1 = block_cls(32, 64)
        self.enc2 = block_cls(64, 128)
        self.enc3 = block_cls(128, 256)
        self.enc4 = block_cls(256, 512)
        self.pool = nn.MaxPool3d(kernel_size=2, stride=2)

        if self.use_attention:
            self.att1 = HARAttentionAblation3D(
                64, depth=1, block_cls=block_cls
            )
            self.att2 = HARAttentionAblation3D(
                128, depth=2, block_cls=block_cls
            )
            self.att3 = HARAttentionAblation3D(
                256, depth=3, block_cls=block_cls
            )
            self.att4 = HARAttentionAblation3D(
                512, depth=4, block_cls=block_cls
            )
        else:
            self.att1 = HARNoAttentionSkip3D(64, block_cls=block_cls)
            self.att2 = HARNoAttentionSkip3D(128, block_cls=block_cls)
            self.att3 = HARNoAttentionSkip3D(256, block_cls=block_cls)
            self.att4 = HARNoAttentionSkip3D(512, block_cls=block_cls)

        self.bottleneck = block_cls(512, 512)

        self.dec4 = block_cls(512 + 512, 512)
        self.dec3 = block_cls(512 + 256, 256)
        self.dec2 = block_cls(256 + 128, 128)
        self.dec1 = block_cls(128 + 64, 64)
        ...
        if self.use_attention:
            ...
            # SE is a channel-attention mechanism, so it is removed in the
            # attention ablation rather than leaving residual attention behind.
    ...
    def _upsample_to(x: torch.Tensor, ref: torch.Tensor) -> torch.Tensor:
        return F.interpolate(
            x, size=ref.shape[2:], mode="trilinear",
            align_corners=False,
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if tuple(original_shape) != working_shape:
            x = F.interpolate(
                x, size=working_shape, mode="trilinear",
                align_corners=False,
            )
        ...
        if tuple(logits.shape[2:]) != tuple(original_shape):
            logits = F.interpolate(
                logits, size=original_shape, mode="trilinear",
                align_corners=False,
            )
        return logits


class HybridAttUnetNoAttention3D(HybridAttUnetAblation3D):
    def __init__(self, in_channels=4, out_channels=4,
                 base_filters=32, internal_size=128):
        super().__init__(
...
class HybridAttUnetNoResidual3D(HybridAttUnetAblation3D):
    def __init__(self, in_channels=4, out_channels=4,
                 base_filters=32, internal_size=128):
        super().__init__(
            in_channels=in_channels,
...
class HybridAttUnetNoAttentionNoResidual3D(HybridAttUnetAblation3D):
    def __init__(self, in_channels=4, out_channels=4,
                 base_filters=32, internal_size=128):
        super().__init__(
            in_channels=in_channels,
...
HYBRIDATT_ABLATION_MODELS = (
    "hybridattunet",
    "hybridattunet_no_attention",
    "hybridattunet_no_residual",
    "hybridattunet_no_attention_no_residual",
)

HYBRIDATT_ABLATION_LABELS = {
    "hybridattunet": "Full HybridAttUnet",
    "hybridattunet_no_attention": "HybridAttUnet - no attention",
    "hybridattunet_no_residual": "HybridAttUnet - no residual blocks",
    "hybridattunet_no_attention_no_residual":
        "HybridAttUnet - no attention + no residual blocks",
}


def hybridatt_ablation_flags(model_name: str):
    model_name = str(model_name).lower()
    if model_name == "hybridattunet":
        return True, True
    if model_name == "hybridattunet_no_attention":
        return False, True
    if model_name == "hybridattunet_no_residual":
        return True, False
    if model_name == "hybridattunet_no_attention_no_residual":
        return False, False
    return None, None
...
MODEL_REGISTRY = {
    ...
    "hybridattunet_no_attention": HybridAttUnetNoAttention3D,
    "hybridattunet_no_residual": HybridAttUnetNoResidual3D,
    "hybridattunet_no_attention_no_residual": HybridAttUnetNoAttentionNoResidual3D,
    "unet3d":          UNet3D,
...
class Trainer:
    def __init__(
        ...
        if (
            ...
            and cfg["model"].lower() in {"unet3d", "hybridattunet", "hybridattunet_no_attention",
            "hybridattunet_no_residual",
            "hybridattunet_no_attention_no_residual", "deepensemble"}
        ):
    ...
    def _autotune_batch_size(self, requested_batch: int) -> int:
        ...
        diff_revision = (
            f"|unet2d_v2|gn{int(cfg.get('unet2d_norm_groups', 8))}"
        ...
        if cfg.get("autobatch_disk_cache", True):
            ...
            cache_path = Path(configured) if configured else Path(cfg.get("save_dir",
...
class FixedSplitRunner:
    ...
    def run(self):
        ...
        combined_row = {
            **segmentation_row,
            **{k: v for k, v in efficiency_row.items() if k != "Model"},
        }
        self._upsert_model_row(combined_path, combined_row)

        # Convenience table for the ablation study. Columns are deliberately
        # identical to model_metrics.csv so no metric definition changes.
        if cfg.get("model", "").lower() in HYBRIDATT_ABLATION_MODELS:
            ablation_path = self.save_dir / "hybridattunet_ablation_metrics.csv"
            self._upsert_model_row(ablation_path, combined_row)
            logger.info(f"HybridAttUnet ablation metrics -> {ablation_path}")

        attention_enabled, residual_enabled = hybridatt_ablation_flags(
            cfg.get("model", "")
        )
        ...
        run_summary = {
            ...
            "best_validation_dice": trainer.best_dice,
            "hybrid_attention_enabled": attention_enabled,
            "hybrid_residual_blocks_enabled": residual_enabled,
...
class CrossValidationRunner:
    ...
    def _eval_checkpoint_on_test(
        ...
        if (
            ...
            and self.cfg["model"].lower() in {"unet3d", "hybridattunet", "hybridattunet_no_attention",
            "hybridattunet_no_residual",
            "hybridattunet_no_attention_no_residual", "deepensemble"}
        ):
...
MODEL_MENU = [
    ("hybridattunet",
     "Full HybridAttUnet 3D (original benchmark network)"),
    ("hybridattunet_no_attention",
     "HybridAttUnet ablation: no attention"),
    ("hybridattunet_no_residual",
     "HybridAttUnet ablation: no residual blocks"),
    ("hybridattunet_no_attention_no_residual",
     "HybridAttUnet ablation: no attention + no residual blocks"),
]
...
def parse_args():
    p = argparse.ArgumentParser(
        description="HybridAttUnet BraTS2020 Ablation Study Pipeline",
    ...
    p.add_argument(
        "--run_all_hybrid_ablation",
        action="store_true",
        help=(
            "Run Full HybridAttUnet followed by the three ablation variants "
            "sequentially on the same fixed split."
        ),
    )
...
def main():
    ...
    if args.list_models:
        ...
        descriptions = {
            "hybridattunet":
                "Full HybridAttUnet 3D - original residual-attention + squeeze-excitation model",
            "hybridattunet_no_attention":
                "Ablation - residual blocks retained; soft-mask attention and SE removed",
            "hybridattunet_no_residual":
                "Ablation - attention/SE retained; residual shortcuts removed from convolutional blocks",
            "hybridattunet_no_attention_no_residual":
                "Ablation - both attention mechanisms and residual block shortcuts removed",
        }
    ...
    if args.mode == "train" and args.run_all_hybrid_ablation:
        # A valid base model is required while common data/cache configuration
        # is resolved. It is replaced sequentially after preprocessing.
        args.model = "hybridattunet"
    ...
    if bool(getattr(args, "run_all_hybrid_ablation", False)):
        if cfg.get("cv"):
            raise RuntimeError(
                "--run_all_hybrid_ablation is designed for the fixed 80/10/10 "
                "experiment, not the legacy CV runner."
            )
        if cfg["mode"] != "train":
            raise RuntimeError(
                "--run_all_hybrid_ablation is only available in training mode."
            )

        logger.info(
            "Starting HybridAttUnet ablation study: full model + three controlled "
            "ablations. All runs reuse the same fixed_split.json."
        )
        for index, model_key in enumerate(HYBRIDATT_ABLATION_MODELS, start=1):
            logger.info("")
            logger.info("#" * 78)
            logger.info(
                f"HYBRIDATT ABLATION {index}/{len(HYBRIDATT_ABLATION_MODELS)}: "
                f"{HYBRIDATT_ABLATION_LABELS[model_key]}"
            )
            logger.info("#" * 78)

            variant_cfg = dict(cfg)
            variant_cfg["model"] = model_key
            variant_cfg["checkpoint"] = None
            FixedSplitRunner(variant_cfg, logger).run()

            gc.collect()
            _safe_cuda_release()

        logger.info(
            "HybridAttUnet ablation study complete. Combined results are in "
            f"{Path(cfg['save_dir']) / 'hybridattunet_ablation_metrics.csv'}"
        )
    elif cfg.get("cv"):
